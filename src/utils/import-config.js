	...
	products: {
		...
		fields: {
			name: {
				required: true,
				validate: (value) =>
					(value && value.length > 0) || "Product name is required",
			},
			shortDescription: {},
			description: {},
			sku: {},
			...
			status: {
				validate: (value) =>
					["active", "draft", "inactive"].includes(value) || "Invalid status",
			},
			barcode: {},
			id: {},
			weight: { type: "number" },
...
/**
 * Processes an entity based on its import configuration
 * Returns processed data and any validation errors
 */
export function processEntityWithConfig(data, entityType) {
	...
	Object.entries(config.fields).forEach(([fieldName, fieldConfig]) => {
		...
		if (value !== undefined) processedData[targetField] = value;
	});

	// Import other fields if configured
	if (config.defaultForOtherFields === "import") {
		Object.entries(data).forEach(([key, value]) => {
			if (!config.fields[key] && value !== undefined) {
	...
	if (config.validateEntity) {
		const entityResult = config.validateEntity(processedData);
