...
const AddOnProductSchema = z.object({
  ...
  category: z.string().min(1, { message: "AddOn Category is required" }),
  mrp: z.coerce.number().min(0, { message: "MRP must be a positive number" }),
  // redeemPoints: z
  // .coerce
  // .number()
...
export function AddOnProductForm({ aadOnProduct, isEditing = false }) {
  const router = useRouter()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [thumbnail, setThumbnail] = useState(aadOnProduct?.thumbnail || "")
  const [images, setImages] = useState(aadOnProduct?.images || [])
  const [videos, setVideos] = useState(aadOnProduct?.videos || [])
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const addonsRes = await axiosInstance.get("/addons");
setAddOnCategories(addonsRes.data?.addons || []);
  ...
  const form = useForm({
    resolver: zodResolver(AddOnProductSchema),
    defaultValues: {
      name: aadOnProduct?.name || "",
      slug: aadOnProduct?.slug || "",
      sku: aadOnProduct?.sku || "",
      barcode: aadOnProduct?.barcode || "",
      googleProductCategory: aadOnProduct?.googleProductCategory || "",
      // flowerType: product?.flowerType || undefined,
      // occasion: product?.occasion || [],
      // color: product?.color || [],
      // fragranceLevel: product?.fragranceLevel || undefined,
      // size: product?.size || undefined,
      category: aadOnProduct?.category?._id || aadOnProduct?.category || "",
      mrp: aadOnProduct?.mrp || 0,
      price: aadOnProduct?.price || 0,
      costPrice: aadOnProduct?.costPrice || 0,
      // redeemPoints: product?.redeemPoints || 0,
      stock: aadOnProduct?.stock || 0,
      lowStockThreshold: aadOnProduct?.lowStockThreshold || 5,
      shortDescription: aadOnProduct?.shortDescription || "",
      description: aadOnProduct?.description || "",
      // giftSetOptions: product?.giftSetOptions || "",
      // deliveryInformation: product?.deliveryInformation || "",
      // careInstructions: product?.careInstructions || "",
      thumbnail: aadOnProduct?.thumbnail || "",
      images: aadOnProduct?.images || [],
      videos: aadOnProduct?.videos || [],
      priority: aadOnProduct?.priority || 0,
      status: aadOnProduct?.status || "draft",
      // featured: aadOnProduct?.featured || false,
      // isArtificial: product?.isArtificial || false,
      // tags: product?.tags || [],
      // variants: aadOnProduct?.variants || [],
      seo: aadOnProduct?.seo || {
        title: "",
        description: "",
        keywords: [],
      },
      allowBackorders: aadOnProduct?.allowBackorders || false,
      metadata: aadOnProduct?.metadata || {},
    },
  })
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      let result;
      if (isEditing && aadOnProduct?._id) {
        result = await axiosInstance.put(`/addons-product/${aadOnProduct._id.toString()}`, processedData);
      } else {
        result = await axiosInstance.post(`/addons-product`, processedData);
      }
  
      if (result.data.success) {
        toast.success(isEditing ? "AddOn-Product updated" : "AddOn-Product created");
        router.push("/admin/addons-products");
      } else {
        toast.error(result.data.error || "Something went wrong");
      }
    } catch (error) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>{isEditing ? "Edit AddOn-Product" : "Add New AddOn-Product"}</CardTitle>
        <CardDescription>
          {isEditing ? "Update the AddOn-product information below" : "Fill in the details to create a new product"}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
...
<FormField
  ...
  render={({ field }) => (
    <FormItem>
      <FormLabel>Add-On Category*</FormLabel>
      <Select
        onValueChange={field.onChange}
        value={field.value}
        disabled={isLoadingCategories || addOnCategories.length === 0}
      >
        <FormControl>
          <SelectTrigger>
            <SelectValue placeholder="Select an add-on category" />
          </SelectTrigger>
        </FormControl>
        <SelectContent>
      ...
      {!isLoadingCategories && addOnCategories.length === 0 && (
        <p className="text-sm text-destructive">
          No add-on categories found.{" "}
          <a href="/admin/addons/new" className="underline">Create one</a>
        </p>
      )}
      <FormMessage />
    </FormItem>
