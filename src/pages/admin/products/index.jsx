...
import GoogleMerchantButton from "@/components/products/GoogleMerchantButton";
...
export default function ProductsPage() {
	...
	const swrKey = `/products?page=${currentPage}${statusQuery}`;
	...
	const handlePageChange = (page) => {
		router.push(`/admin/products?page=${page}${statusQuery}`);
	};

	const handleTabChange = (value) => {
		setActiveTab(value);
		const statusParam = value !== "all" ? `&status=${value}` : "";
		router.push(`/admin/products?page=1${statusParam}`);
	};
